list(params = list(class_distribution = list(
    ...
    or_cnet_plot = list(showCategory = 3, displayed_labels = "IDs and Description", 
        ...
        color = "p.adjust", size = "Count", score_threshold = 0.2, 
        similarity_score = "JC", edge_magnifier = 1, node_magnifier = 0.1, 
        enable_physics = FALSE)), hardcoded_settings = list(enrichment_analysis = list(
    terms = c("Gene ontology (ALL)", "Gene ontology (BP)", "Gene ontology (MF)", 
    "Gene ontology (CC)"), adjustment = c("holm", "hochberg", 
    "hommel", "bonferroni", "BH", "BY", "fdr", "none")), over_representation_analysis = list(
    terms = c("Gene ontology (ALL)", "Gene ontology (BP)", "Gene ontology (MF)", 
    "Gene ontology (CC)"), adjustment = c("holm", "hochberg", 
