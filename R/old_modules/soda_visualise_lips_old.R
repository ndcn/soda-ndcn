    ...
    if (id_plot == "spawn_class_distribution"){
      ...
      output$sidebar_pca_ui = shiny::renderUI({
        shiny::tagList(
          ...
          shiny::selectInput(
            inputId = ns("sidebar_pca_metacol"),
      ...
      shiny::observeEvent(c(input$sidebar_pca_dataset, input$sidebar_pca_metacol), {
        ...
        r6$plot_pca(data_table = data_table,
                    col_group = input$sidebar_pca_metacol,
                    width = reactive_xpx * x_plot,
                    height = reactive_ypx * y_plot,
                    colour_list = colour_list)
...
soda_visualise_lips_server = function(id, r6, colour_list) {
  shiny::moduleServer(
    ...
    function(input, output, session) {
      ...
      dimensions_obj$y_box = 0.7
      dimensions_obj$x_plot = 0.8
      dimensions_obj$y_plot = 0.65
